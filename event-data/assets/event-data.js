(($) => {
  ...
  function getEventTickets() {
    get('/ticket-tailor/event/ev_410363/tickets').done((data) => {
      console.log(data);
    });
  }

  // Ticket tailor events.
  $('#event-data-get-ticket-tailor-events').click(() => {
    get('/ticket-tailor/events').done((data) => {
      const table = new Tabulator("#event-data-ticket-tailor-events", {
        layout: 'fitDataFill',
        columns: [
          { title: 'Date', field: 'date', sorter: 'string', hozAlign:"center" },
          { title: 'Event', field: 'event', sorter: 'string' },
          { title: 'Id', field: 'id', sorter: 'string', hozAlign:"center", headerTooltip: 'This is what we want to be displayed as the header' },
          { title: 'Published', field: 'published', sorter: 'boolean', formatter: 'tickCross', hozAlign:"center" },
  ...
  function loadTicketsForTicketTailorEvent(eventId) {
    get('/ticket-tailor/event/' + eventId + '/tickets').done((data) => {
      ...
      const table = new Tabulator("#event-data-ticket-tailor-tickets", {
        layout: 'fitDataFill',
        columns: [
          { title: 'Issued', field: 'issued', sorter: 'string', hozAlign:"center" },
          { title: 'Type', field: 'type', sorter: 'string' },
          { title: 'Email', field: 'email', sorter: 'string' },
          { title: 'First name', field: 'firstName', sorter: 'string' },
          { title: 'Last name', field: 'lastName', sorter: 'string' },
          { title: 'Id', field: 'id', sorter: 'string', hozAlign:"center", headerTooltip: 'This is what we want to be displayed as the header' },
          { title: 'Valid', field: 'isValid', sorter: 'boolean', formatter: 'tickCross', hozAlign:"center" },
        ...
        data: data.tickets.map((ticket) => {
          ...
          ticket.custom_questions.forEach(({ question, answer }, i) => {
